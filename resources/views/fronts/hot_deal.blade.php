...
@section('content')
    ...
    <!-- //short-->
    <!--show Now-->
    <section class="contact pt-lg-4 pt-md-3 pt-sm-3 pt-3 pb-3" id="products">
        <div class="container pt-lg-5 pt-md-4 pt-sm-4 pt-3" id="product_cart">
            <h5 class="text-dark py-3">
                {{ 'Hot Products' }} <small style="font-size: small">-
                    <span class="text-danger">{{ count($count_product) }}</span> টি পণ্য পাওয়া গেছে</small>
            </h5>
                                    ...
                                    <div class="col-sm-12 col-xs-12" style="padding:0;background: cornsilk;">
                                        ...
                                        <span id="productPrice209" class="col-sm-12  col-xs-12 text-center"
                                              style="padding: 0;display: block;height: 23px;line-height:28px;color: #355088;font-size: 14px;font-weight: bold">
                                                ...
                                                ৳{{ $v_allProduct->offer_price }}
                                                <del style="color:red"> ৳{{ $v_allProduct->price }}</del>
                                            @else
                                                ৳{{ $v_allProduct->price }}
                                            @endif
                                            </span>
                                            ...
                                            <span class="col-sm-6  col-xs-6 text-center">
                                                  @if(Auth::check())
                                                    <a class="text-decoration-none"
                                                       href="{{ url('order-now') }}/{{ $v_allProduct->id }}"
                                                ...
                                                @else
                                                    <a class="text-decoration-none" data-toggle="modal"
                                                       data-target="#orderNowSingleProductModal-{{ $v_allProduct->id }}"
                                                       href=""
                                                       style="background:none;border: none;margin: 0;padding: 0;font-weight:bold;color:#fff">
                                                            Order Now
                                                        </a>
